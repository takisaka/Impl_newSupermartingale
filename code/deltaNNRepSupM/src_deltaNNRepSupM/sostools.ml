...
let sostools_of_reachability_problem (((pcfg_transition, l_domain, v_domain), (l_init,init_valuation), invariant, term_config) as problem) (delta: float option) (nn: int) (template_deg: int) (sos_deg: int) (sdp_program_name: string) (positivstellensatz : positivstellensatz) (sostools_opt: bool) (solver: string) (vsdp: bool): string =

  let template, template_param_domain = generate_polynomial_template l_domain v_domain template_deg in
  

  let sequents, new_v_list, new_params, objective_func= sequents_and_objective_func_of_reachability_problem problem delta nn template in
(*  let objective_func = get_objective_func template l_init init_valuation v_domain in *)

  let template_param_domain =
    List.fold_left
      (fun l a -> add_param_to_param_domain l a)
