...
let progmath_of_reachability_problem (((pcfg_transition, l_domain, v_domain), (l_init,init_valuation), invariant, term_config) as problem: reachability_problem) (delta: float option) (nn: int) : string =
  let template, template_param_domain = generate_linear_template l_domain v_domain in
  let sequents, new_vars_list, new_params, objective_func = sequents_and_objective_func_of_reachability_problem problem delta nn template in
