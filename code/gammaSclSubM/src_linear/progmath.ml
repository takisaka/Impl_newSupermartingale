...
let progmath_of_reachability_problem (((pcfg_transition, l_domain, v_domain), (l_init,init_valuation), invariant, term_config) as problem: reachability_problem) (gamma: float) : string =
  let template, template_param_domain = generate_linear_template l_domain v_domain in
  let sequents, new_vars_list = sequents_of_reachability_problem problem (c_coef_polynomial_of_c_coef (c_coef_of_float gamma)) true_and_ineq_list template in
