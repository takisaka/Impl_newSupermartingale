...
let epsilon_param = param_of_string_temporary "epsilon"
let kappa_param = param_of_string_temporary "kappa"
let epsilon_param_string = string_of_param epsilon_param
let kappa_param_string = string_of_param kappa_param
...
let sostools_of_reachability_problem (((pcfg_transition, l_domain, v_domain), (l_init,init_valuation), invariant, term_config) as problem) (epsilon: float option) (kappa: const_type) (template_deg: int) (sos_deg: int) (function_name: string) (sdp_program_name: string) (positivstellensatz : positivstellensatz) (sostools_opt: bool) : string =
  let template, template_param_domain = generate_polynomial_template l_domain v_domain template_deg in

  let kappa_p_coef_polynomial, const_var_list, template_param_domain, objective_func = match kappa with
    | MatVar ->
       (p_coef_polynomial_of_p_coef (p_coef_of_param kappa_param),
        [kappa_param],
        template_param_domain,
        get_objective_func template l_init init_valuation v_domain
       )
    | Param ->
       (p_coef_polynomial_of_p_coef (p_coef_of_param kappa_param),
        [],
        add_param_to_param_domain template_param_domain kappa_param,
        p_coef_of_param kappa_param
       )
    | Eq s ->
       (p_coef_polynomial_of_p_coef @@ p_coef_of_float s,
        [],
        template_param_domain,
        get_objective_func template l_init init_valuation v_domain
       )  in
  let epsilon_p_coef_polynomial, const_var_list = match epsilon with
    | None -> (p_coef_polynomial_of_p_coef (p_coef_of_param epsilon_param), epsilon_param::const_var_list)
    | Some s -> (p_coef_polynomial_of_p_coef @@ p_coef_of_float s, const_var_list) in

  
  let sequents, new_v_list = sequents_and_objective_func_of_reachability_problem problem epsilon_p_coef_polynomial kappa_p_coef_polynomial template in
  ...
  let buf = Buffer.create (1024 * 1024) in
  buffer_add_line buf ("function [sol, bound] = " ^ function_name ^ "(" ^ string_of_list_infix_string string_of_param const_var_list ", " ^ ")");
  
(*  if List.length const_var_list > 0 then buffer_add_line buf ("clearvars -except "^ string_of_list_infix_string string_of_v const_var_list " " ^ ";"); *)
  buffer_add_line buf "";
  ...
  buffer_add_line buf "% getting a solution";
  buffer_add_line buf sostools_solve;
  buffer_add_line buf "";
  buffer_add_line buf "";


  if not (kappa = Param) then
  (buffer_add_line buf "% calculate a probability bound";
  (match epsilon with
   | None -> ()
   | Some s -> buffer_add_line buf (string_of_param epsilon_param ^ " = " ^ string_of_float s ^ ";"));
  (match kappa with
   | Eq s -> buffer_add_line buf (string_of_param kappa_param ^ " = " ^ string_of_float s ^ ";")
   | Param | MatVar -> ());
  buffer_add_line buf ("gamma = exp(-(" ^ string_of_param epsilon_param ^ "^2)/(2*(" ^ string_of_param kappa_param ^ "+" ^ string_of_param epsilon_param ^ ")^2));");
  buffer_add_line buf ("alpha = exp(("  ^ string_of_param epsilon_param ^ "*sol)/(("  ^ string_of_param kappa_param ^  "+" ^ string_of_param epsilon_param ^  ")^2));");
  buffer_add_line buf "";
  buffer_add_line buf ("bound = alpha * (gamma^(ceil(sol/" ^ string_of_param kappa_param ^ "))) / (1-gamma)"));
